...
export const MODULE_ADDRESS = "0xed101e6c098f47d3a9ff8cf2dae4331fc2a55848502942246878b2ab63b90b4d";
export const DEPLOYMENT_TX_HASH = "0x76223771c7f1720f65b1d1300659d101867676a996e225406b6aa10292eb258d";
...
export async function waitForTransaction(transactionHash: string) {
    ...
    if (!response.success) {
        ...
        console.error("\n🔎 Error Analysis:");
        if (vmStatus.includes("EESCROW_ALREADY_EXISTS") || abortCodeMatch?.[1] === "5") {
            console.error("  → Escrow already exists for this match_id");
        } else if (vmStatus.includes("EESCROW_NOT_FOUND") || abortCodeMatch?.[1] === "4") {
            ...
            console.error("  → Resource does not exist (EscrowStore might not be initialized)");
