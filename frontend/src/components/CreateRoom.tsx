...
import { useMatch } from "../hooks/blockchain/useMatch";
...
export default function CreateRoom({
  ...
  console.log({ chains });
  const { createMatch: createOnchainMatch } = useMatchActions();
  ...
  const handleCreateRoom = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dbMatchId = matchResponse.id; // string

      // 2️⃣ Create escrow + market onchain
      const tx = await createOnchainMatch(
        1234123,
      ...
      <form onSubmit={handleCreateRoom} className="space-y-4">
        ...
        <button
          ...
          className={`w-full py-3 px-4 rounded-lg font-semibold transition-colors ${
            loading || !entryFee || !opponentWallet || !opponentName
